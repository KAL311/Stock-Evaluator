...
def load_annual_with_fallback(
    ...
    if 'Shares (Diluted)' in merged_i.columns:
        bad_sh = merged_i['Shares (Diluted)'].notna() & (
            merged_i['Shares (Diluted)'].abs() < 100_000
        )
        if bad_sh.any():
            for (tk, rd), row in merged_i[bad_sh].iterrows():
                dq_offenders.append({
                    'ticker': tk, 'report_date': rd, 'field': 'Shares (Diluted)',
    ...
    if dq_offenders and verbose:
        try:
            ...
            _dq_path = _dq_dir / f'input_dq_{pd.Timestamp.now().strftime("%Y%m%d")}.csv'
            ...
            print(
                f"    input DQ guard: NULLed rows in memory but log write failed ({e})"
            )
